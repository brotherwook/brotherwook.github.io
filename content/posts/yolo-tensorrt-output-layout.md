...
date: 2026-09-14T14:20:00+09:00
draft: false
categories: \["AI/ML"]
...
내용:  \[cx, cy, w, h, class\_score\_0, class\_score\_1, ...]
```
...
```python
output = raw\_output.reshape(1, 5, -1)   # (1, 4+nc, anchors)를 전제
```
...
self.output\_shape = tuple(output\_binding\_shape)
# (1, N, 6)이면 end2end, 아니면 raw
self.end2end = len(self.output\_shape) == 3 and self.output\_shape\[-1] == 6

print(f"OUTPUT SHAPE: {self.output\_shape} -> "
...
```python
output = raw\_output.reshape(self.output\_shape)

if self.end2end:
    # (1, N, 6): 이미 NMS 완료. conf 필터만.
    dets = output\[0]
    keep = dets\[:, 4] > conf\_thres
    boxes  = dets\[keep, :4]
    scores = dets\[keep, 4]
    classes = dets\[keep, 5]
else:
...
boxes\_xywh = np.column\_stack(\[
    boxes\_xyxy\[:, 0],                      # x
    boxes\_xyxy\[:, 1],                      # y
    boxes\_xyxy\[:, 2] - boxes\_xyxy\[:, 0],   # w
    boxes\_xyxy\[:, 3] - boxes\_xyxy\[:, 1],   # h
])
indices = cv2.dnn.NMSBoxes(boxes\_xywh.tolist(), scores.tolist(), conf, iou)
```
