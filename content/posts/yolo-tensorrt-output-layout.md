---
title: "[YOLO] TensorRT로 바꿨더니 검출이 0개? — 출력 레이아웃 함정"
...
YOLO 모델을 TensorRT 엔진으로 변환해서 추론 코드에 물렸다. 변환도 성공, 엔진 로드도 되고, 실행하면 프로세스가 멀쩡히 돈다. 로그에 예외도 없다. 그런데 **검출 결과가 0개다.** 분명 화면엔 객체가 있는데 하나도 안 잡힌다.

이런 종류의 버그가 제일 고약하다. 에러가 나면 스택 트레이스라도 따라가는데, **아무 일도 안 일어난 것처럼 조용히 틀린 결과**가 나오면 어디부터 봐야 할지 막막하다.

결론부터 말하면, 원인은 **ONNX export 때 출력 텐서 레이아웃이 바뀌었는데 후처리 코드가 옛날 레이아웃을 그대로 가정하고 있던 것**이었다. 그리고 그 잘못된 가정이 **에러조차 안 내고 통과**해버린 게 문제를 어렵게 만들었다.
...
NMS(Non-Maximum Suppression)가 적용되지 않은 원본 예측이다. 수천 개 앵커마다 박스 좌표와 클래스 스코어가 들어 있고, **후처리에서 직접 스코어 필터링 + NMS를 수행**해야 최종 박스가 나온다.
...
NMS까지 모델 그래프 안에서 끝낸 결과다. 이미 정리된 최대 N개 박스가 나오고, 남는 슬롯은 0으로 패딩된다. 후처리는 **conf 임계값 필터만** 걸면 된다.
...
구버전에서는 export의 `nms` 옵션이 특정 포맷 전용이라 ONNX엔 적용되지 않는 경우가 있었다. 그래서 `nms=True`를 줘도 여전히 raw `(1, 4+nc, anchors)`가 나왔다. 그런데 버전을 올리자 `nms=True`가 의도대로 동작하기 시작해, 같은 코드·같은 옵션인데 **출력이 end2end `(1, N, 6)`으로 바뀌어버렸다.**
...
원래 raw 출력을 5행(좌표 4 + 클래스 1)으로 펴려는 의도다. 그런데 출력이 end2end `(1, 300, 6)`으로 바뀌면 전체 원소 수는 `300 * 6 = 1800`개다. 그리고..
...
추측으론 못 잡는다. 결국 **엔진을 로드해서 출력 텐서의 shape과 실제 값을 그대로 찍어봤다.**
...
이제 시작 로그의 `OUTPUT SHAPE:` 한 줄만 보면 어느 경로를 타는지 즉시 안다. 다음에 또 export 방식이 바뀌어도 코드는 그대로 두면 된다.

## 보너스 함정: `cv2.dnn.NMSBoxes`는 xywh를 받는다

raw 경로에서 NMS를 직접 돌릴 때 또 하나 실수하기 쉬운 지점이 있다. OpenCV의 NMS 함수는 박스를 **`[x, y, w, h]` 형식으로 받는다.** xyxy(좌상단·우하단 좌표)를 그대로 넘기면 `x2`·`y2`를 너비·높이로 오해해서 박스가 실제보다 훨씬 크게 잡힌다. 그러면 IoU가 과대평가되고, 겹치지도 않은 정상 객체까지 억제돼 검출 수가 줄어든다.
...
- YOLO의 ONNX 출력은 `nms` 옵션에 따라 **raw `(1,4+nc,A)`** 와 **end2end `(1,N,6)`** 두 레이아웃이 있고, 후처리가 완전히 다르다.
...
"에러가 안 나는 버그"를 만났을 때의 교훈은 하나로 모인다 — **중간 산출물(여기선 엔진 출력 텐서)을 가정하지 말고 직접 찍어봐라.** 그 한 번의 `print`가 며칠을 아낀다.
