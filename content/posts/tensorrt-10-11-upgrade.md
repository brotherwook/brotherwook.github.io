...
좋은 점은, 변환 스크립트와 런타임이 **정밀도에 무관**해진다는 것이다. `trtexec`는 ONNX dtype을 그대로 따르므로 fp16이든 fp32든 같은 명령으로 변환되고, 런타임도 엔진에서 dtype을 읽어(`trt.nptype(get_tensor_dtype(name))`) 버퍼를 할당하면 분기 없이 동작한다. **정밀도를 바꾸고 싶으면 export의 `half`만 고치고 다시 만들면 끝**이다.
...
`create_execution_context`를 호출하는 객체가 `None`이라는 건, 그 앞 단계인 `deserialize_cuda_engine()`이 **엔진 객체 대신 `None`을 돌려줬다**는 뜻이다. 그런데 예외를 던지는 게 아니라 **조용히 `None`을 반환**하니 원인이 한눈에 안 보인다.
